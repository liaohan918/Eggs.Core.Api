    ...
    public class BlogArticleServices : BaseServices<BlogArticle>, IBlogArticleServices
    {
        ...
        public async Task<BlogViewModels> GetBlogDetails(int id)
        {
            ...
            if (blogArticle != null)
            {
                ...
                string bcategory = blogArticle.bcategory;//类别

                //要取下一篇和上一篇，以当前id开始，按id排序后top(2)，而不用取出所有记录
                //这样在记录很多的时候也不会有多大影响
                var nextBlogs = await base.Query(a => a.bID >= id && a.IsDeleted == false && a.bcategory == bcategory, 2, "bID");
                if (nextBlogs.Count == 2)
                {
                    models.next = nextBlogs[1].btitle;
                    models.nextID = nextBlogs[1].bID;
                }
                var prevBlogs = await base.Query(a => a.bID <= id && a.IsDeleted == false && a.bcategory == bcategory, 2, "bID desc");
