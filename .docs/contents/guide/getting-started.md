...
## 下载
Github（国际） 下载 [https://github.com/anjoy8/Eggs.Core.Api](https://github.com/anjoy8/Eggs.Core.Api)  
    
Gitee（国内） 下载 [https://gitee.com/laozhangIsPhi/Eggs.Core.Api](https://gitee.com/laozhangIsPhi/Eggs.Core.Api)  


## 编译与运行
1、拿到项目后，双击 `Eggs.Core.Api.sln` 解决方案；  
2、首先 `F6` 编译，看是否有错误；  
...
1、打开 `Eggs.Core.Api` 项目下的 `appsettings.json` 文件；  
...
## 如何配置项目端口号
1、在 `Eggs.Core.Api` 层下的 `program.cs` 文件中，将 `8081`端口，修改为自己想要的端口号；    
...
## 新增实体模块后如何迁移到数据库
1、在 `Eggs.Core.Api.Model` 项目目录下的 `Seed` 文件夹下，找到 `DBSeed` 类；  
...
## 新增数据库表，如何反向生成四层文件
1、可以通过 `T4` 模板来生成，在 `Eggs.Core.Api.FrameWork` 层，使用方法: [9757999.html](https://www.cnblogs.com/laozhang-is-phi/p/9757999.html#autoid-4-3-0) ；  
...
## 发布与部署
1、双击项目根目录下的 `Eggs.Core.Api.Publish.bat`批处理文件；  
