...
namespace Eggs.Core.Api.Extensions
{
    ...
    public static class AllServicesMildd
    {
        public static void UseAllServicesMildd(this IApplicationBuilder app, IServiceCollection _services)
        {
            ...
            tsDIAutofac.AddRange(Assembly.LoadFrom(Path.Combine(AppContext.BaseDirectory, "Eggs.Core.Api.Services.dll")).GetTypes().ToList());
            tsDIAutofac.AddRange(Assembly.LoadFrom(Path.Combine(AppContext.BaseDirectory, "Eggs.Core.Api.Repository.dll")).GetTypes().ToList());
